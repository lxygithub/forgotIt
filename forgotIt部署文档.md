...
### 5.4 端侧 AI（v1.5 起，三级降级链的第一级）

除了服务端端点，应用支持把轻量模型直接跑在**用户浏览器里**（WebGPU 推理，Web Worker 中执行），
构成三级降级链——**设备内 WebLLM → 服务端 AI（上面三种方式配置的端点）→ 静默兜底**：
...
服务端接入自建 Ollama 的提示（第二级自定义）：Ollama 自带 OpenAI 兼容层，界面配置里
端点填 `http://<内网或隧道地址>:11434/v1`、模型填如 `qwen2.5vl:7b` 即可；若经 Cloudflare Tunnel
暴露公网，务必配套 Cloudflare Access/WAF 鉴权（Ollama 本身无认证）。
